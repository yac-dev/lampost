...
const ReportAsset = (props) => {
  const inputAccessoryViewID = 'REPORT_ASSET';
  ...
  const onSubmitPress = async () => {
    // ここでapiを送ろう。
    const payload = {
      assetId: props.route.params.assetId,
      userId: auth.data._id,
      issue: selectedIssue,
      description,
    };
    setLoading(true);
    const result = await lampostAPI.post('/reports/asset', payload);
    console.log(payload);
    setLoading(false);
    setSelectedIssue({});
    setDescription('');
    ...
    >
      <ScrollView>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
          ...
          <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 20 }}>Reporting an asset</Text>
        </View>
        <Text style={{ color: 'white', marginBottom: 10, fontWeight: 'bold', fontSize: 17 }}>
          What kind of issue with this asset?
        </Text>
